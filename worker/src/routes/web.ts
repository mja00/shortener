...
import { generateUniqueAlias, randomString } from "../lib/alias";
...
web.post("/register", async (c) => {
  ...
  if (disabled) return c.redirect("/", 302);
...
web.get("/", async (c) => {
  ...
  if (rootRedirect && !user) return c.redirect(rootRedirect, 302);
...
web.get("/links/info/:id", requireSession, async (c) => {
  ...
  if (!link) return c.json({ error: "Short link not found" });
