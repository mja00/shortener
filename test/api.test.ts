...
describe('GET list routes', () => {
	it('splits active/expired/deleted buckets', async () => {
		...
		expect((active.links as { short_url: string; }[]).map(link => link.short_url)).toEqual(['active']);
		expect(Object.keys(expired)).toEqual(['links']);
		expect((expired.links as { short_url: string; }[]).map(link => link.short_url)).toEqual(['gone']);
		expect(Object.keys(deleted)).toEqual(['links']);
		expect((deleted.links as { short_url: string; }[]).map(link => link.short_url)).toEqual(['trashed']);
	});
});

describe('POST /api/links', () => {
	const create = (payload: unknown) => linkGet('/links', {
		method: 'POST',
	...
	it('normalizes epoch-second expiration to ISO', async () => {
		const res = await create({ url: 'https://example.org/x', alias: 'epoch', expiration_date: 1_700_000_000 });
		expect(res.status).toBe(201);
...
describe('PUT /api/links/:id', () => {
	const update = (id: number, payload: unknown) => linkGet(`/links/${id}`, {
		method: 'PUT',
		headers: { ...AUTH, 'Content-Type': 'application/json' },
		body: JSON.stringify(payload),
	});
...
describe('PUT /api/links/:id/restore', () => {
	it('clears deleted and expired', async () => {
		...
		expect(active.map(link => link.short_url)).toEqual(['revive']);
	});
