...
async function pbkdf2Derive(
	...
	const hashLen = algo === 'SHA-512' ? 64 : 32;
	...
	for (let block = 1; block <= blocks; block++) {
		...
		const mac = await crypto.subtle.sign(
			'HMAC', key, concatBytes(saltBytes, blockBytes),
		);
		let prev = new Uint8Array(mac);
		const xorAcc = new Uint8Array(prev);
		for (let i = 1; i < iterations; i++) {
			const next = new Uint8Array(await crypto.subtle.sign('HMAC', key, prev));
			for (const [index, element] of next.entries()) {
				xorAcc[index] = (xorAcc[index] ?? 0) ^ element;
			}
			prev = next;
		}
		const offset = (block - 1) * hashLen;
		out.set(xorAcc.subarray(0, Math.min(xorAcc.length, dkLen - offset)), offset);
	}
...
async function verifyPbkdf2(password: string, hash: string): Promise<boolean> {
	...
	const [head, salt, expected] = dollar;
	const [method, iterStr] = (head ?? '').slice('pbkdf2:'.length).split(':', 2);
	if (!method || !iterStr || !salt || !expected) {
		return false;
	}
